...
public class BankBrancheServiceImpl implements BankBrancheService {
    ...
    @Override
    public BankBranche findBankBrancheByName(String name) {

        System.out.println("Service - Finding bank branches by name");
        Optional<BankBranche> bankBranches = null;

        try {
            bankBranches = bankBranchRepository.findBankBrancheByName(name);

        if(!bankBranches.isPresent()){
